...
from rLGG_package.rLGG_treatment_simulation import chemotherapy_simulation, chemotherapy_stupp_simulation
...
def solve_ode(y0, param):
    ...
    Y_global, t_global = chemotherapy_stupp_simulation(rLGG_model_v3, Y_global, 
                                                       t_global, treatment['c'], param)
    ...
    # between treatment ---------------------------------------------    
    y0 = get_last_y_values(Y_global)
    t_between_treatment = np.arange(treatment['c2'][1], treatment['c3'][0], tstep)
    res_between_treatment = odeint(rLGG_model_v3, y0, t_between_treatment, 
                                args=(param, ))
    Y_global = np.vstack((Y_global, res_between_treatment))
    t_global = np.vstack((t_global, t_between_treatment.reshape(-1, 1)))
    ...
    # treatment - chemotherapy  -------------------------------------
    Y_global, t_global = chemotherapy_simulation(rLGG_model_v3, Y_global, 
                                                 t_global, treatment['c3'], param)
    
    # post-traitement -----------------------------------------------
    y0 = get_last_y_values(Y_global)
    t_post_treatment = np.arange(treatment['c3'][1], tf, tstep) 
...
for i in range(n_opti):
    ...
    tau = np.random.uniform(1e-2, 0.1)
    psi = np.random.uniform(0.1, 1)
    a1 =  np.random.uniform(0.15, 0.5)
...
# Final plot 
plot_fit(t_global, Y_global, data_time, data_volume, treatment, p_id, save_img=False, ylim=200, errorbar=False)
